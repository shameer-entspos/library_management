    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            'group/sidebar-wrapper has-data-[variant=inset]:dark:bg-sidebar flex min-h-svh w-full has-data-[variant=inset]:bg-neutral-100',
            className
          )}
          {...props}
        >
  ...
  if (collapsible === 'none') {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          'dark:bg-sidebar text-sidebar-foreground flex h-full w-(--sidebar-width) flex-col bg-neutral-100',
          className
        )}
        {...props}
      >
    ...
    >
      ...
      >
        <div
          ...
          className="dark:bg-sidebar group-data-[variant=floating]:border-sidebar-border flex h-full w-full flex-col bg-neutral-100 group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm"
        >
          {children}
        </div>
      </div>
...
function SidebarInset({ className, ...props }: React.ComponentProps<'main'>) {
  return (
    <main
      ...
      className={cn(
        'bg-background scroll-hide relative flex w-full flex-1 flex-col',
...
const sidebarMenuButtonVariants = cva(
  'peer/menu-button flex w-full items-center gap-2 overflow-hidden cursor-pointer rounded-lg p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-5 [&>svg]:shrink-0 transition duration-100 ease-in',
  {
    variants: {
      variant: {
        default: 'hover:bg-primary/30 hover:text-sidebar-accent-foreground',
        outline:
          'bg-primary/80 text-white dark:bg-primary hover:text-white hover:bg-primary/80 font-bold ',
      },
      size: {
